...
class LAScriterionFirstReturnOnly : public LAScriterion
{
public:
  inline const char* name() const { return "first_only"; };
...
class LAScriterionLastReturnOnly : public LAScriterion
{
public:
  inline const char* name() const { return "last_only"; };
  inline int get_command(char* string) const { return sprintf(string, "-%s ", name()); };
...
void LASfilter::usage() const
{
  ...
  fprintf(stderr,"  -first_only\n");
  fprintf(stderr,"  -last_only\n");
...
BOOL LASfilter::parse(int argc, char* argv[])
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    {
      add_criterion(new LAScriterionFirstReturnOnly());
      *argv[i]='\0';
    }
    else if (strcmp(argv[i],"-last_only") == 0)
    {
      add_criterion(new LAScriterionLastReturnOnly());
      *argv[i]='\0';
    }
