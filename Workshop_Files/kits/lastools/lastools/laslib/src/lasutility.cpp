...
BOOL LASoccupancyGrid::write_asc_grid(const char* file_name) const
{
  ...
  /*
BOOL LASoccupancyGrid::read(ByteStreamIn* stream)
{
  char signature[4];
  if (!stream->getBytes((U8*)signature, 4))
  {
    fprintf(stderr,"ERROR (LASoccupancyGrid): reading signature\n");
    return FALSE;
  }
  U32 version;
  if (!stream->get32bitsLE((U8*)&version))
  {
    fprintf(stderr,"ERROR (LASoccupancyGrid): reading version\n");
    return FALSE;
  }
  ...
  */
  ...
  /*
BOOL LASoccupancyGrid::write(ByteStreamOut* stream) const
{
  if (!stream->putBytes((U8*)"LASO", 4))
  {
    fprintf(stderr,"ERROR (LASoccupancyGrid): writing signature\n");
    return FALSE;
  }
  U32 version = 0;
  if (!stream->put32bitsLE((U8*)&version))
  {
    fprintf(stderr,"ERROR (LASoccupancyGrid): writing version\n");
    return FALSE;
  }
