...
void LASindex::complete(U32 minimum_points, I32 maximum_intervals)
{
  ...
  if (minimum_points)
  {
    ...
    {
      cell_hash[hash1][interval->index] = interval->total;
    }
    while (cell_hash[hash1].size())
    {
      I32 hash2 = (hash1+1)%2;
      cell_hash[hash2].clear();
      // coarsen if a coarser cell will still have fewer than minimum_points 
        ...
        if ((*hash_element_outer).second)
        {
          if (spatial->coarsen((*hash_element_outer).first, &coarser_index, &num_indices, &indices))
          {
            ...
            for (i = 0, j = 0; i < num_indices; i++)
            {
              ...
              {
                total += (*hash_element_inner).second;
            ...
            {
              interval->merge_cells(j, indices, coarser_index);
              coarsened = TRUE;
            }
            cell_hash[hash2][coarser_index] = total;
          }
        }
        hash_element_outer++;
      }
...
void LASindex::print(BOOL verbose)
{
  ...
  while (interval->has_cells())
  {
    ...
    if (verbose) fprintf(stderr,"cell %d intervals %d full %d %d (%.2f) \n", interval->index, intervals, full, interval->total, 100.0f*full/total);
    total_cells++;
    total_full += full;
    total_total += total;
...
{
  have_interval = FALSE;
  U32 num_cells = spatial->intersect_rectangle(r_min_x, r_min_y, r_max_x, r_max_y);
...
{
  have_interval = FALSE;
  U32 num_cells = spatial->intersect_tile(ll_x, ll_y, size);
...
{
  have_interval = FALSE;
  U32 num_cells = spatial->intersect_circle(center_x, center_y, radius);
...
BOOL LASindex::merge_intervals()
{
  if (spatial->get_intersected_cells())
  {
    U32 used_cells = 0;
    U32 empty_cells = 0;
    while (spatial->has_intersected_cells())
    {
      if (interval->get_cell(spatial->intersected_cell))
      {
        interval->add_cell_to_merge(spatial->intersected_cell);
        used_cells++;
      }
      else
      {
