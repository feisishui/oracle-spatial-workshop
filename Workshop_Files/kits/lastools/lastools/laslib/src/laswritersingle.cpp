...
BOOL LASwriterSingle::refile(FILE* file)
{
...
BOOL LASwriterSingle::open(const LASheader* header, U32 compressor, I32 requested_version, I32 chunk_size)
{
  // create output
...
BOOL LASwriterSingle::open(const char* file_name, const LASheader* header, U32 compressor, I32 requested_version, I32 chunk_size)
{
  if (file_name == 0)
  {
...
BOOL LASwriterSingle::open(FILE* file, const LASheader* header, U32 compressor, I32 requested_version, I32 chunk_size)
{
  if (file == 0)
  {
...
BOOL LASwriterSingle::open(ostream& stream, const LASheader* header, U32 compressor, I32 requested_version, I32 chunk_size)
{
  // create output
  ByteStreamOut* out;
  if (IS_LITTLE_ENDIAN())
...
BOOL LASwriterSingle::open(ByteStreamOut* stream, const LASheader* header, U32 compressor, I32 requested_version, I32 chunk_size)
{
  U32 i;
  ...
  if (header->laszip)
  {
    if (!point.init(&quantizer, header->laszip->num_items, header->laszip->items)) return FALSE;
    point_is_standard = header->laszip->is_standard(&point_data_format, &point_data_record_length);
  }
  else
  {
    if (!point.init(&quantizer, header->point_data_format, header->point_data_record_length)) return FALSE;
  ...
  header_start_position = stream->position();

  // reset byte counter so we know how many bytes we put into the stream
...
//      fprintf(stderr,"WARNING: wrong header->vlrs[%d].reserved: %d != 0xAABB\n", i, header->vlrs[i].reserved);
  ...
  if (laszip)
  {
    ...
    char description[40];
    sprintf(description, "by laszip of LAStools v%d\0\0\0\0", LAS_TOOLS_VERSION);  
  ...
  if (header->vlr_lastiling)
  {
    ...
    U8 user_id[16] = "finalized tile\0";
    if (!stream->putBytes((U8*)user_id, 16))
    {
      fprintf(stderr,"ERROR: writing user_id %s\n", user_id);
      return FALSE;
    ...
    {
      fprintf(stderr,"ERROR: writing header->vlr_lastiling->level %d\n", (I32)header->vlr_lastiling->level);
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(header->vlr_lastiling->level_index)))
    {
      fprintf(stderr,"ERROR: writing header->vlr_lastiling->level_index %d\n", (I32)header->vlr_lastiling->level_index);
      return FALSE;
    }
    if (!stream->put32bitsLE((U8*)&(header->vlr_lastiling->implicit_levels)))
    {
      fprintf(stderr,"ERROR: writing header->vlr_lastiling->implicit_levels %d\n", (I32)header->vlr_lastiling->implicit_levels);
      return FALSE;
    }
...
BOOL LASwriterSingle::write_point(const LASpoint* point)
{
  p_count++;
  return writer->write(point->point);
}

BOOL LASwriterSingle::chunk()
{
  return writer->chunk();
}

BOOL LASwriterSingle::update_header(const LASheader* header, BOOL use_inventory)
{
  U32 i;
  if (header == 0)
  {
...
U32 LASwriterSingle::close(BOOL update_header)
{
  U32 bytes = 0;
...
LASwriterSingle::LASwriterSingle()
{
  file = 0;
  stream = 0;
  writer = 0;
