  ...
  if (projection)
  {
    if (projection->type == GEO_PROJECTION_UTM || projection->type == GEO_PROJECTION_LCC || projection->type == GEO_PROJECTION_TM)
    {
      ...
      else if (projection->type == GEO_PROJECTION_LCC)
      {
        ...
        (*geo_keys)[5].tiff_tag_location = 0;
        (*geo_keys)[5].count = 1;
      ...
      else if (projection->type == GEO_PROJECTION_TM)
      {
        ...
        (*geo_keys)[7].count = 1;
        (*geo_keys)[7].value_offset = 3;
        (*geo_double_params)[3] = tm->tm_false_easting_meter;

        (*geo_keys)[8].key_id = 3083; // ProjFalseNorthingGeoKey
        (*geo_keys)[8].tiff_tag_location = 0;
        (*geo_keys)[8].count = 1;
        (*geo_keys)[8].value_offset = 4;
        (*geo_double_params)[4] = tm->tm_false_northing_meter;
...
bool GeoProjectionConverter::set_projection_from_geo_keys(int num_geo_keys, GeoProjectionGeoKeys* geo_keys, char* geo_ascii_params, double* geo_double_params, char* description)
{
  ...
  if (!has_projection)
  {
    if (user_defined_projection == 1)
    {
      ...
      {
        double falseEasting = geo_double_params[offsetProjFalseEastingGeoKey];
        double falseNorthing = geo_double_params[offsetProjFalseNorthingGeoKey];
        ...
        set_transverse_mercator_projection(falseEasting, falseNorthing, latOriginDeg, longMeridianDeg, scaleFactor);
        if (description)
        {
    ...
    else if (user_defined_projection == 8)
    {
          ...
          (offsetProjStdParallel2GeoKey >= 0))
      {
        ...
        set_lambert_conformal_conic_projection(falseEasting, falseNorthing, latOriginDeg, longOriginDeg, firstStdParallelDeg, secondStdParallelDeg);
        if (description)
        {
...
bool GeoProjectionConverter::parse(int argc, char* argv[])
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if (strcmp(argv[i],"-lcc") == 0 || strcmp(argv[i],"-lambert_conic_conformal") == 0 || strcmp(argv[i],"-target_lcc") == 0)
    {
      ...
      if (strcmp(argv[i+3],"survey_feet") == 0 || strcmp(argv[i+3],"survey_foot") == 0 || strcmp(argv[i+3],"surveyfeet") == 0 || strcmp(argv[i+3],"surveyfoot"))
      {
    ...
    else if (strcmp(argv[i],"-tm") == 0 || strcmp(argv[i],"-traverse_mercator") == 0 || strcmp(argv[i],"-target_tm") == 0)
    {
      bool source = (strcmp(argv[i],"-tm") == 0 || strcmp(argv[i],"-traverse_mercator") == 0);
      double falseEasting; sscanf(argv[i+1], "%lf", &falseEasting);
      double falseNorthing; sscanf(argv[i+2], "%lf", &falseNorthing);
...
int GeoProjectionConverter::unparse(char* string) const
{
  ...
  if (source_projection != 0)
  {
    ...
    else if (source_projection->type == GEO_PROJECTION_LCC)
    {
      ...
      n += sprintf(&string[n], "-lcc %g %g m %g %g %g %g ", lcc->lcc_false_easting_meter, lcc->lcc_false_northing_meter, lcc->lcc_lat_origin_degree, lcc->lcc_long_meridian_degree, lcc->lcc_first_std_parallel_degree, lcc->lcc_second_std_parallel_degree);
    }
    else if (source_projection->type == GEO_PROJECTION_TM)
    {
      GeoProjectionParametersTM* tm = (GeoProjectionParametersTM*)source_projection;
      n += sprintf(&string[n], "-tm %g %g m %g %g %g ", tm->tm_false_easting_meter, tm->tm_false_northing_meter, tm->tm_lat_origin_degree, tm->tm_long_meridian_degree, tm->tm_scale_factor);
    }
  }
  if (ellipsoid)
  {
...
double GeoProjectionConverter::target_precision() const
{
