...
void usage(bool wait=false)
{
  ...
  fprintf(stderr,"lasinfo -nv -i lidar.las\n");
  fprintf(stderr,"lasinfo -nv -nc -stdout lidar.las\n");
  fprintf(stderr,"lasinfo -nv -nc -stdout *.las | grep version\n");
  fprintf(stderr,"lasinfo -i lidar.las -o lidar_info.txt\n");
  fprintf(stderr,"lasinfo -i lidar.las -repair\n");
  fprintf(stderr,"lasinfo -i lidar.las -repair_bounding_box -set_file_creation 8 2007\n");
  fprintf(stderr,"lasinfo -i lidar.las -set_version 1.2\n");
  fprintf(stderr,"lasinfo -i lidar.las -set_system_identifier \"hello world!\" -set_generating_software \"this is a test (-:\"\n");
  if (wait)
  {
...
int main(int argc, char *argv[])
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if (strcmp(argv[i],"-v") == 0 || strcmp(argv[i],"-verbose") == 0 || strcmp(argv[i],"-peep") == 0)
    {
      verbose = true;
    }
    ...
    else if (strcmp(argv[i],"-nv") == 0 || strcmp(argv[i],"-no_vlr") == 0 || strcmp(argv[i],"-no_var") == 0)
    {
      no_variable_header = true;
    }
    else if (strcmp(argv[i],"-nmm") == 0 || strcmp(argv[i],"-no_minmax") == 0 || strcmp(argv[i],"-no_min_max") == 0)
    {
      no_min_max = true;
    }
    ...
    else if (strcmp(argv[i],"-nc") == 0 || strcmp(argv[i],"-nocheck") == 0 || strcmp(argv[i],"-no_check") == 0)
    {
      check_points = false;
    }
    ...
    else if (strcmp(argv[i],"-stdout") == 0 || strcmp(argv[i],"-otxt") == 0)
    {
      file_out = stdout;
    }
    ...
    else if (strcmp(argv[i],"-set_system_identifier") == 0 || strcmp(argv[i],"-system_identifier") == 0 || strcmp(argv[i],"-sys_id") == 0)
    {
      ...
      strncpy(set_system_identifier, argv[i], 31);
      set_system_identifier[31] = '\0';
      change_header = true;
		}
    else if (strcmp(argv[i],"-set_generating_software") == 0 || strcmp(argv[i],"-generating_software") == 0 || strcmp(argv[i],"-gen_soft") == 0)
    {
      ...
      strncpy(set_generating_software, argv[i], 31);
      set_generating_software[31] = '\0';
      change_header = true;
		}
    else if (strcmp(argv[i],"-set_global_encoding") == 0 || strcmp(argv[i],"-global_encoding") == 0)
    {
			i++;
      set_global_encoding = (U16)atoi(argv[i]);
      change_header = true;
		}
    ...
    {
			i++;
      file_creation_day = (U16)atoi(argv[i]);
			i++;
      file_creation_year = (U16)atoi(argv[i]);
      change_header = true;
		}
    else if (strcmp(argv[i],"-set_header_size") == 0 || strcmp(argv[i],"-header_size") == 0)
    {
			i++;
      set_header_size = atoi(argv[i]);
      change_header = true;
    }
    else if (strcmp(argv[i],"-set_offset_to_point_data") == 0 || strcmp(argv[i],"-offset_to_point_data") == 0)
    {
			i++;
      set_offset_to_point_data = atoi(argv[i]);
      change_header = true;
    }
    else if (strcmp(argv[i],"-set_start_of_waveform_data_packet_record") == 0 || strcmp(argv[i],"-start_of_waveform_data_packet_record") == 0)
    {
			i++;
  ...
  while (lasreadopener.active())
  {
...
#ifdef _WIN32
    if (verbose) fprintf(stderr, "%s '%s' with %I64d points\n", (repair_header ? "repairing" : "reading"), (lasreadopener.get_file_name() ? lasreadopener.get_file_name() : "stdin"), lasreader->npoints);
#else
    ...
    {
#ifdef _WIN32
      if (lasreader->npoints > number_of_point_records) if (file_out) fprintf(file_out, "extended LAS file has %I64d points, more than 32 bits can count.\012", lasreader->npoints);
#else
      if (lasreader->npoints > number_of_point_records) if (file_out) fprintf(file_out, "extended LAS file has %lld points, more than 32 bits can count.\012", lasreader->npoints);
#endif
      if (file_out) fprintf(file_out, "reporting all LAS header entries:\012");
      if (file_out) fprintf(file_out, "  file signature:            '%.4s'\012", lasheader->file_signature);
      if (file_out) fprintf(file_out, "  file source ID:            %d\012", lasheader->file_source_id);
      if (file_out) fprintf(file_out, "  reserved (global_encoding):%d\012", lasheader->global_encoding);
      if (file_out) fprintf(file_out, "  project ID GUID data 1-4:  %u %d %d '%.8s'\012", lasheader->project_ID_GUID_data_1, lasheader->project_ID_GUID_data_2, lasheader->project_ID_GUID_data_3, lasheader->project_ID_GUID_data_4);
      if (file_out) fprintf(file_out, "  version major.minor:       %d.%d\012", lasheader->version_major, lasheader->version_minor);
      if (file_out) fprintf(file_out, "  system identifier:         '%.32s'\012", lasheader->system_identifier);
      if (file_out) fprintf(file_out, "  generating software:       '%.32s'\012", lasheader->generating_software);
      if (file_out) fprintf(file_out, "  file creation day/year:    %d/%d\012", lasheader->file_creation_day, lasheader->file_creation_year);
      if (file_out) fprintf(file_out, "  header size                %d\012", lasheader->header_size);
      if (file_out) fprintf(file_out, "  offset to point data       %u\012", lasheader->offset_to_point_data);
      if (file_out) fprintf(file_out, "  number var. length records %u\012", lasheader->number_of_variable_length_records);
      if (file_out) fprintf(file_out, "  point data format          %d\012", lasheader->point_data_format);
      if (file_out) fprintf(file_out, "  point data record length   %d\012", lasheader->point_data_record_length);
      if (file_out) fprintf(file_out, "  number of point records    %u\012", lasheader->number_of_point_records);
      if (file_out) fprintf(file_out, "  number of points by return %u %u %u %u %u\012", lasheader->number_of_points_by_return[0], lasheader->number_of_points_by_return[1], lasheader->number_of_points_by_return[2], lasheader->number_of_points_by_return[3], lasheader->number_of_points_by_return[4]);
      if (file_out) fprintf(file_out, "  scale factor x y z         %g %g %g\012", lasheader->x_scale_factor, lasheader->y_scale_factor, lasheader->z_scale_factor);
      if (file_out) fprintf(file_out, "  offset x y z               "); lidardouble2string(printstring, lasheader->x_offset); fprintf(file_out, "%s ", printstring);  lidardouble2string(printstring, lasheader->y_offset); fprintf(file_out, "%s ", printstring);  lidardouble2string(printstring, lasheader->z_offset); fprintf(file_out, "%s\012", printstring);
      if (file_out) fprintf(file_out, "  min x y z                  "); lidardouble2string(printstring, lasheader->min_x, lasheader->x_scale_factor); fprintf(file_out, "%s ", printstring); lidardouble2string(printstring, lasheader->min_y, lasheader->y_scale_factor); fprintf(file_out, "%s ", printstring); lidardouble2string(printstring, lasheader->min_z, lasheader->z_scale_factor); fprintf(file_out, "%s\012", printstring);
      if (file_out) fprintf(file_out, "  max x y z                  "); lidardouble2string(printstring, lasheader->max_x, lasheader->x_scale_factor); fprintf(file_out, "%s ", printstring); lidardouble2string(printstring, lasheader->max_y, lasheader->y_scale_factor); fprintf(file_out, "%s ", printstring); lidardouble2string(printstring, lasheader->max_z, lasheader->z_scale_factor); fprintf(file_out, "%s\012", printstring);
      if (file_out) if ((lasheader->version_major == 1) && (lasheader->version_minor >= 3)) fprintf(file_out, "  start_of_waveform_data_packet_record %d\012", (I32)lasheader->start_of_waveform_data_packet_record);
      if (file_out) if (lasheader->user_data_in_header_size) fprintf(file_out, "the header contains %u user-defined bytes\012", lasheader->user_data_in_header_size);
    }

    // maybe print variable header

    if (!no_variable_header)
    {
      for (int i = 0; i < (int)lasheader->number_of_variable_length_records; i++)
      {
        if (file_out) fprintf(file_out, "variable length header record %d of %d:\012", i+1, (int)lasheader->number_of_variable_length_records);
        if (file_out) fprintf(file_out, "  reserved             %d\012", lasreader->header.vlrs[i].reserved);
        if (file_out) fprintf(file_out, "  user ID              '%s'\012", lasreader->header.vlrs[i].user_id);
        if (file_out) fprintf(file_out, "  record ID            %d\012", lasreader->header.vlrs[i].record_id);
        if (file_out) fprintf(file_out, "  length after header  %d\012", lasreader->header.vlrs[i].record_length_after_header);
        if (file_out) fprintf(file_out, "  description          '%s'\012", lasreader->header.vlrs[i].description);
        ...
        if (strcmp(lasheader->vlrs[i].user_id, "LASF_Projection") == 0)
        {
          ...
          {
            if (file_out) fprintf(file_out, "    GeoKeyDirectoryTag version %d.%d.%d number of keys %d\012", lasheader->vlr_geo_keys->key_directory_version, lasheader->vlr_geo_keys->key_revision, lasheader->vlr_geo_keys->minor_revision, lasheader->vlr_geo_keys->number_of_keys);
          ...
          {
            if (file_out) fprintf(file_out, "    GeoDoubleParamsTag (number of doubles %d)\012", lasreader->header.vlrs[i].record_length_after_header/8);
            if (file_out) fprintf(file_out, "      ");
            for (int j = 0; j < lasreader->header.vlrs[i].record_length_after_header/8; j++)
            {
              if (file_out) fprintf(file_out, "%lg ", lasheader->vlr_geo_double_params[j]);
            }
            if (file_out) fprintf(file_out, "\012");
          }
          else if (lasheader->vlrs[i].record_id == 34737) // GeoAsciiParamsTag
          {
            if (file_out) fprintf(file_out, "    GeoAsciiParamsTag (number of characters %d)\012", lasreader->header.vlrs[i].record_length_after_header);
            if (file_out) fprintf(file_out, "      ");
            for (int j = 0; j < lasreader->header.vlrs[i].record_length_after_header; j++)
            {
              if (file_out) fprintf(file_out, "%c", lasheader->vlr_geo_ascii_params[j]);
            }
            if (file_out) fprintf(file_out, "\012");
          }
        }
        else if (strcmp(lasheader->vlrs[i].user_id, "LASF_Spec") == 0)
        {
          if (lasheader->vlrs[i].record_id == 0) // ClassificationLookup
          {
            ...
            for (int j = 0; j < 256; j++)
            {
              if (file_out) fprintf(file_out, "%d %s", vlr_classification[j].class_number, vlr_classification[j].description);
            }
          }
          ...
          else if (lasheader->vlrs[i].record_id == 3) // TextAreaDescription
          {
            ...
            {
              if (file_out) fprintf(file_out, "%c", lasheader->vlrs[i].data[j]);
            }
            if (file_out) fprintf(file_out, "\012");
          }
          else if ((lasheader->vlrs[i].record_id >= 100) && (lasheader->vlrs[i].record_id < 355)) // WavePacketDescriptor
          {
            ...
            if (file_out) fprintf(file_out, "  index %d bits/sample %d compression %d samples %u temporal %u gain %lg, offset %lg\012", lasheader->vlrs[i].record_id-99, vlr_wave_packet_descr->getBitsPerSample(), vlr_wave_packet_descr->getCompressionType(), vlr_wave_packet_descr->getNumberOfSamples(), vlr_wave_packet_descr->getTemporalSpacing(), vlr_wave_packet_descr->getDigitizerGain(), vlr_wave_packet_descr->getDigitizerOffset());
          }
        }
      }
    }
    if (!no_header && file_out) if (lasheader->user_data_after_header_size) fprintf(file_out, "the header is followed by %u user-defined bytes\012", lasheader->user_data_after_header_size);
    ...
    if (check_points)
    {
      if (!no_min_max && file_out) fprintf(file_out, "reporting minimum and maximum for all LAS point record entries ...\012");
...
#ifdef _WIN32
        if (progress && (lasreader->p_count % progress) == 0) if (file_out) fprintf(file_out, " ... processed %I64d points ...\012", lasreader->p_count);
#else
        if (progress && (lasreader->p_count % progress) == 0) if (file_out) fprintf(file_out, " ... processed %lld points ...\012", lasreader->p_count);
#endif
      }
      if (!no_min_max)
      {
        if (file_out) fprintf(file_out, "  x %d %d\012",lassummary.min.x, lassummary.max.x);
        if (file_out) fprintf(file_out, "  y %d %d\012",lassummary.min.y, lassummary.max.y);
        if (file_out) fprintf(file_out, "  z %d %d\012",lassummary.min.z, lassummary.max.z);
        if (file_out) fprintf(file_out, "  intensity %d %d\012",lassummary.min.intensity, lassummary.max.intensity);
        if (file_out) fprintf(file_out, "  edge_of_flight_line %d %d\012",lassummary.min.edge_of_flight_line, lassummary.max.edge_of_flight_line);
        if (file_out) fprintf(file_out, "  scan_direction_flag %d %d\012",lassummary.min.scan_direction_flag, lassummary.max.scan_direction_flag);
        if (file_out) fprintf(file_out, "  number_of_returns_of_given_pulse %d %d\012",lassummary.min.number_of_returns_of_given_pulse, lassummary.max.number_of_returns_of_given_pulse);
        if (file_out) fprintf(file_out, "  return_number %d %d\012",lassummary.min.return_number, lassummary.max.return_number);
        if (file_out) fprintf(file_out, "  classification %d %d\012",lassummary.min.classification, lassummary.max.classification);
        if (file_out) fprintf(file_out, "  scan_angle_rank %d %d\012",lassummary.min.scan_angle_rank, lassummary.max.scan_angle_rank);
        if (file_out) fprintf(file_out, "  user_data %d %d\012",lassummary.min.user_data, lassummary.max.user_data);
        if (file_out) fprintf(file_out, "  point_source_ID %d %d\012",lassummary.min.point_source_ID, lassummary.max.point_source_ID);
        if (lasreader->point.have_gps_time)
        {
          if (file_out) fprintf(file_out, "  gps_time %f %f\012",lassummary.min.gps_time, lassummary.max.gps_time);
        }
        if (lasreader->point.have_rgb)
        {
          if (file_out) fprintf(file_out, "  Color R %d %d\012", lassummary.min.rgb[0], lassummary.max.rgb[0]);
          if (file_out) fprintf(file_out, "        G %d %d\012", lassummary.min.rgb[1], lassummary.max.rgb[1]);
          if (file_out) fprintf(file_out, "        B %d %d\012", lassummary.min.rgb[2], lassummary.max.rgb[2]);
        }
        if (lasreader->point.have_wavepacket)
        {
          if (file_out) fprintf(file_out, "  Wavepacket Index    %d %d\012", lassummary.min.wavepacket.getIndex(), lassummary.max.wavepacket.getIndex());
          if (file_out) fprintf(file_out, "             Offset   %d %d\012", (I32)lassummary.min.wavepacket.getOffset(), (I32)lassummary.max.wavepacket.getOffset());
          if (file_out) fprintf(file_out, "             Size     %d %d\012", lassummary.min.wavepacket.getSize(), lassummary.max.wavepacket.getSize());
          if (file_out) fprintf(file_out, "             Location %g %g\012", lassummary.min.wavepacket.getLocation(), lassummary.max.wavepacket.getLocation());
          if (file_out) fprintf(file_out, "             Xt       %g %g\012", lassummary.min.wavepacket.getXt(), lassummary.max.wavepacket.getXt());
          if (file_out) fprintf(file_out, "             Yt       %g %g\012", lassummary.min.wavepacket.getYt(), lassummary.max.wavepacket.getYt());
          if (file_out) fprintf(file_out, "             Zt       %g %g\012", lassummary.min.wavepacket.getZt(), lassummary.max.wavepacket.getZt());
        }
        if (number_of_point_records == 0 && lasheader->number_of_point_records > 0)
        {
          if (file_out) fprintf(file_out, "re-reporting LAS header entries populated during read pass:\012");
          if (file_out) fprintf(file_out, "  number of point records    %u\012", lasheader->number_of_point_records);
          if (file_out) fprintf(file_out, "  number of points by return %u %u %u %u %u\012", lasheader->number_of_points_by_return[0], lasheader->number_of_points_by_return[1], lasheader->number_of_points_by_return[2], lasheader->number_of_points_by_return[3], lasheader->number_of_points_by_return[4]);
          if (file_out) fprintf(file_out, "  min x y z                  "); lidardouble2string(printstring, lasheader->min_x, lasheader->x_scale_factor); fprintf(file_out, "%s ", printstring); lidardouble2string(printstring, lasheader->min_y, lasheader->y_scale_factor); fprintf(file_out, "%s ", printstring); lidardouble2string(printstring, lasheader->min_z, lasheader->z_scale_factor); fprintf(file_out, "%s\012", printstring);
          if (file_out) fprintf(file_out, "  max x y z                  "); lidardouble2string(printstring, lasheader->max_x, lasheader->x_scale_factor); fprintf(file_out, "%s ", printstring); lidardouble2string(printstring, lasheader->max_y, lasheader->y_scale_factor); fprintf(file_out, "%s ", printstring); lidardouble2string(printstring, lasheader->max_z, lasheader->z_scale_factor); fprintf(file_out, "%s\012", printstring);
    ...
    if (change_header)
    {
      if (set_global_encoding)
      {
        fseek(file, 6, SEEK_SET);
        fwrite(&set_global_encoding, sizeof(U16), 1, file);
      }
      ...
      if (file_creation_day || file_creation_year)
      {
        ...
        fwrite(&file_creation_day, sizeof(U16), 1, file);
        fwrite(&file_creation_year, sizeof(U16), 1, file);
      }
      if (set_header_size)
      {
    ...
    if (check_points)
    {
      if (lassummary.number_of_point_records != lasheader->number_of_point_records)
      {
        if (repair_header)
        {
          ...
          fwrite(&lassummary.number_of_point_records, sizeof(U32), 1, file);
          if (file_out) fprintf(file_out, "real number of points (%u) is different from header number of points (%u). repairing ...\n", lassummary.number_of_point_records, lasheader->number_of_point_records);
        }
        else
        {
          if (!no_min_max && file_out) fprintf(file_out, "real number of points (%u) is different from header number of points (%u)\n", lassummary.number_of_point_records, lasheader->number_of_point_records);
        }
      }

      bool report = false;
      bool was_set = false;
      for (i = 1; i < 6; i++) if (lasheader->number_of_points_by_return[i-1] != lassummary.number_of_points_by_return[i]) report = true;
      ...
      if (report)
      {
        if (!no_min_max)
        {
          ...
          for (i = 1; i < 6; i++) if (file_out) fprintf(file_out, " %u", lassummary.number_of_points_by_return[i]); 
          if (file_out) fprintf(file_out, " %s\n", repair_header ? "(repaired)" : "");
        }
        if (repair_header)
        {
          fseek(file, 111, SEEK_SET);
          fwrite(&(lassummary.number_of_points_by_return[1]), sizeof(U32), 5, file);
        }
      }

      if (!no_min_max)
      {
        if (lassummary.number_of_points_by_return[0]) if (file_out) fprintf(file_out, "WARNING: there are %u points with return number 0\n", lassummary.number_of_points_by_return[0]); 
        if (lassummary.number_of_points_by_return[6]) if (file_out) fprintf(file_out, "WARNING: there are %u points with return number 6\n", lassummary.number_of_points_by_return[6]); 
        if (lassummary.number_of_points_by_return[7]) if (file_out) fprintf(file_out, "WARNING: there are %u points with return number 7\n", lassummary.number_of_points_by_return[7]); 

        report = false;
        for (i = 1; i < 8; i++) if (lassummary.number_of_returns_of_given_pulse[i]) report = true;
        if (report)
        {
          if (file_out) fprintf(file_out, "overview over number of returns of given pulse:"); 
          for (i = 1; i < 8; i++) if (file_out) fprintf(file_out, " %u", lassummary.number_of_returns_of_given_pulse[i]);
          if (file_out) fprintf(file_out, "\n"); 
        }

        if (lassummary.number_of_returns_of_given_pulse[0]) if (file_out) fprintf(file_out, "WARNING: there are %u points with a number of returns of given pulse of 0\n", lassummary.number_of_returns_of_given_pulse[0]); 

        report = false;
        for (i = 0; i < 32; i++) if (lassummary.classification[i]) report = true;
        if (lassummary.classification_synthetic || lassummary.classification_keypoint ||  lassummary.classification_withheld) report = true;

        if (report)
        {
          if (file_out) fprintf(file_out, "histogram of classification of points:\n"); 
          for (i = 0; i < 32; i++) if (lassummary.classification[i]) if (file_out) fprintf(file_out, " %8d %s (%u)\n", lassummary.classification[i], LASpointClassification[i], i);
          if (lassummary.classification_synthetic) if (file_out) fprintf(file_out, " +-> flagged as synthetic: %u\n", lassummary.classification_synthetic);
          if (lassummary.classification_keypoint) if (file_out) fprintf(file_out,  " +-> flagged as keypoints: %u\n", lassummary.classification_keypoint);
          if (lassummary.classification_withheld) if (file_out) fprintf(file_out,  " +-> flagged as withheld:  %u\n", lassummary.classification_withheld);
        }
      }
