 ...
 * number.
 *
 * Column names (found in the input DBF file) are checked against a
 * list of reserved words. If a name is found to be reserved, then "_MB" gets
 * appended to the name. For example "DATE" becomes "DATE_M".
 *
 * The list of reserved words is obtained by query:
...
public class SdoLoadShape
{
  ...
  public static void main(String args[]) throws Exception
  {
    ...
    if (args.length < 8) {
      ...
      System.out.println ("<Shape File>:   Name of shape file (without .SHP extension)");
    ...
    boolean createTable       = (args.length > 8 ? (Integer.parseInt(args[8])==1) : false);
    int     commitFrequency   = (args.length > 9 ? Integer.parseInt(args[9]) : 0);
    boolean fastPickle        = (args.length > 10 ? (Integer.parseInt(args[10])==1) : false);
    boolean verbose           = (args.length > 11 ? (Integer.parseInt(args[11])==1) : false);
    ...
    loadGeometries(dbConnection, tableName, geoColumn, idColumn, shapeFileName, srid, commitFrequency, createTable, fastPickle, verbose);

    // Close database connection
    dbConnection.close();
  }

  public static void loadShapeFile(String shapeFileName, int srid, String tableName)
  throws Exception {
    loadShapeFile(shapeFileName, srid, tableName, "GEOMETRY", "ID", 1, 1000, 1, 0);
  }
  public static void loadShapeFile(String shapeFileName, int srid, String tableName, String geoColumn, String idColumn, int createTable)
  throws Exception {
    loadShapeFile(shapeFileName, srid, tableName, geoColumn, idColumn, createTable, 1000, 1, 0);
  }
  ...
  throws Exception {
    // Get a connection to the database
    Connection dbConnection = new oracle.jdbc.driver.OracleDriver ().defaultConnection();

    // Import the geometries
    loadGeometries(dbConnection, tableName, geoColumn, idColumn, shapeFileName, srid, commitFrequency,
      (createTable == 1 ? true : false), (fastPickle == 1 ? true : false), (verbose == 1 ? true : false) );
  ...
  static void loadGeometries(
    Connection dbConnection, String tableName, String geoColumn, String idColumn, String shapeFileName, int srid, int commitFrequency,
    boolean createTable, boolean fastPickle, boolean verbose
  )
    throws Exception
  {
    ...
    for (int i=0; i<numFields; i++) {
      ...
      // Append "_M" to column names that are reserved words
      for (int j=0; j<reservedWords.length; j++)
        if (reservedWords[j].equals(fieldName[i]))
          fieldName[i] = fieldName[i] + "_M";
    }
